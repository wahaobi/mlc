...
var BlockTemplate = module.exports = function BlockTemplate(jobId, rpcData, extraNoncePlaceholder, reward, recipients, poolAddress, coin){
    ...
    //console.log(this.rpcData.coinbasetxn.data)
    ...
    /*if (typeof this.genTx === 'undefined') {
        this.genTx = new Buffer(rpcData.coinbasetxn.data, 'hex').toString('hex');
	console.log(this.genTx);
	//this.genTx = transactions.createGeneration(rpcData.height, blockReward, this.rewardFees, recipients, poolAddress, coin).toString('hex');
        //console.log(this.genTx);
        this.genTxHash = new Buffer(rpcData.coinbasetxn.hash, 'hex').toString('hex');
	console.log(this.genTxHash);
    ...
    this.merkleRoot = merkle.getRoot(rpcData);
