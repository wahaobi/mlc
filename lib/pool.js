            ...
            if (options.coin.disablecb) {
              ...
              _this.daemon.cmd('setpubkey', [pubkey], function (results) {
                    if (results.error) {
                      ...
                      var pubkeyaddress = results[0].response.address;
                      if ( pubkeyaddress !== options.address) {
                          emitErrorLog('The pool address ' +
                               options.address + ' does not match the pubkey set in the daemon.')
                               return;
                      }
                    }
                });
